/**
 * Note: This file has only slight additions to the existing code by Main
 * @author: Chirag Huria
...
public class ObjectNode
{
     ...
     **/
    // Time Complexity: Big Theta - Theta(1)
     ...
     * @precondition
     * The linked list to be copied from is valid with each node being an
     * object of the ObjectNode and the source parameter being a reference
     * to the head of this list.
     * @postcondition
     * A new list is created from the existing list where the nodes from
     * the source list is copied to the new list while preserving
     * the order, and reference to head.
     *
     **/
    // Time Complexity: Big Theta - Theta(N)
     ...
     * A new list is created from the existing list where the nodes from
     * the source list is copied to the new list while preserving
     * the order, reference to head, and reference to tail.
     *
     **/
    // Time Complexity: Big Theta - Theta(N)
    ...
    public static ObjectNode listCopy_rec(ObjectNode source)
    {
        ...
        // Boundary condition for the recursive stack calls to return
        // This will also take care of non-existent list
        if (source == null)
            return null;

        // The below recursive call will ensure that the data from the current ObjectNode reference
        // is copied into the iteratively building list (when the stack calls return), and the
        // recursive function call is made with the reference link to the next node in the list
        ObjectNode buildList = new ObjectNode(source.data, listCopy_rec(source.link));
    ...
    public String displayEveryThird(){
      ...
      StringBuilder displayEveryThirdString = new StringBuilder();
      int i;
      for(i=1; i<=elements && cursor!=null; i++, cursor = cursor.link){
            if(i%3==0){
                displayEveryThirdString.append(cursor.data.toString());
            }
      }
      return displayEveryThirdString.toString();
    }
    ...
    public String toString(){
        ...
        for( ; cursor!=null; cursor=cursor.link){
            resultString.append(cursor.data.toString());
        }
